...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "--models" || a === "-m") out.models = args[++i];
    else if (a === "--config" || a === "-c") out.config = args[++i];
    else if (a === "--timeout" || a === "-t") out.timeout = Number(args[++i]) || 5000;
    else if (a === "--backup-dir" || a === "-b") out.backupDir = args[++i];
  }
  return out;
}
...
async function main() {
  ...
  if (bak) console.log(`Backed up ${configPath} -> ${bak}`);
